...
export class DashboardReportes implements OnInit {
  ...
  // ── Drill-down context ──────────────────────────────────────────
  drillDownContext: {
    tipo: 'examen' | 'servicio' | 'doctor' | 'diagnostico' | 'sede' | 'paciente' | 'seguro' | 'sexo' | 'trazabilidad' | 'resultado' | null;
    valor: string;
    titulo: string;
    datos: any[];
  } | null = null;
  ...
  onChartClick(event: any, tipo: string) {
    ...
    if (tipo === 'servicio') {
      ...
      const areaEncontrada = this.areas.find(a => 
  ...
  buildGlobales(d: any) {
    ...
    this.sedeChart = this.hBar((d.sedes ?? []).filter((s:any) => s.sede?.trim()), 'sede', '#F97316', 26);
    this.cie10Chart = this.buildCIE10Chart((d.diagnosticos_cie10 ?? []).slice(0,12));
    this.trazabilidadChart = this.buildTrazabilidadChart(d.kpis?.total_solicitados ?? 0, d.kpis?.total_con_resultado ?? 0);
    ...
    this.pacientesChart = this.hBar((d.pacientes_frecuentes ?? []).slice(0, 10), 'paciente', '#F59E0B', 25);
    ...
    this.sedesTreemapChart = this.buildSedesTreemapChart((d.sedes ?? []).filter((s:any) => s.sede?.trim()));
  }
  ...
  buildArea(d: any) {
    ...
    this.areaGaugeChart = this.buildGaugeChart(this.areaTasa, this.areaConResultado, this.areaSolicitados);
    ...
    this.areaExamenResultadoChart = this.buildExamenResultadoChart(a, this.areaConResultado, this.areaSolicitados);
    this.areaDiaSemanaChart = this.buildDiaSemanaChart(a.dia_semana ?? []);
    this.areaPacientesChart = this.hBar((a.pacientes_frecuentes ?? []).slice(0, 10), 'paciente', '#F59E0B', 25);
  ...
  private mostrarDrillDown(tipo: string, valor: string) {
    ...
    const tipoValido = tipo as 'examen' | 'doctor' | 'diagnostico' | 'sede' | 'paciente' | 'seguro' | 'sexo' | 'trazabilidad' | 'resultado';
    
    switch(tipoValido) {
      case 'examen':
        titulo = `Detalle del examen: ${valor}`;
        // Buscar en los datos reales o usar simulación
        datos = this.rawData?.examenes_mas_solicitados?.filter((e: any) => 
          e.desc_examen?.toLowerCase().includes(valor.toLowerCase())
        )?.map((e: any) => ({
  ...
  isEmpty(data: any[]): boolean { return !data || data.length === 0; }
    ...
    return {
      ...
      series: [
        ...
        },
        {
          ...
          stack: 'total',
          barMaxWidth: 40,
          data: examTop.map((e: any) => Math.round(e.total * (conResultado / Math.max(totalArea, 1)))),
    ...
    return {
      ...
      radar: {
        indicator: [
          ...
          { name: 'Vie', max: maxVal }, { name: 'Sáb', max: maxVal }, 
          { name: 'Dom', max: maxVal }
        ],
      ...
      series: [{
        ...
        data: [{
          value: [
            dataObj['Lunes'], dataObj['Martes'], dataObj['Miércoles'],
            dataObj['Jueves'], dataObj['Viernes'], dataObj['Sábado'], dataObj['Domingo']
          ],
          name: 'Volumen',
